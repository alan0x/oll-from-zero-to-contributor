...
- 通过 `connect` 建立教学关系的 Geometry、Plot、Scene3D 等主视觉要按实际尺寸相邻放置，不能只让箭头跨越一大片空白；
- 聚焦某个公式、文字或注释时，要把与它最近的主视觉一起纳入画面；聚焦一个相连主视觉时，要保留它的另一端；
- 宿主把工具栏、任务卡、旁白、输入框和教师形象的屏幕矩形作为遮挡区传给相机，Runtime 在剩余可见区域内取景。

因此，“节点坐标合法”不等于“讲课时用户看得见”。布局和相机取景必须联合验收。
...
输入权不等于播放暂停。学生的 slider、geometry point、Scene3D、画布平移和缩放可以与旁白并存；老师只有在演示同一个变量时才临时取得该变量的输入权。
...
框选只定义来源，不移动原稿。选择边框、控制点和工具栏可以变化，但选中的 Ink 路径保持原世界坐标。
...
重播课程时，前端可以暂时不显示既有 Ink，避免它混入老师重新播放的板书。重播结束必须把之前的 Ink 和本次重播期间新增的 Ink 合并回当前文档，而不是永久隐藏或覆盖任何一份原稿。
