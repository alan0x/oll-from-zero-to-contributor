...
- [ ] `capabilities.ts` 只在 Runtime 真正支持后才更新。
- [ ] 相连主视觉、focus 和宿主遮挡通过真实浏览器观察。
...
- [ ] mock Provider 测试通过。
- [ ] 真实 eval 覆盖自然用户表达。
- [ ] 复核模型失败或不同意不会取消本地合法课件。
- [ ] 没有依赖模型固定措辞、标签或无条件“通过”。
- [ ] 对模型等价写法的整理按 OLL 结构泛化，不按题目关键词枚举。
- [ ] 性能和重试次数有记录。
...
- [ ] 文件交付可恢复。
- [ ] Skill Action 在能力协商后调用，旧服务端有明确失败提示。
- [ ] 会话媒体路径经过真实路径和边界检查。
- [ ] 超时、取消、重试分类正确。
- [ ] 未泄露环境变量或凭据。
